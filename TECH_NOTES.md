# ServicePortal MVP - Technical Notes
...
I utilized the **Gemini API** (`gemini-2.5-flash`) to add a "Smart Reply" feature in the messaging component.
*   **Feature:** Clicking the magic wand (✨) reads the job context and generates a polite, professional follow-up message for the customer.
*   **Benefit:** Reduces friction for the customer when communicating with service staff.

## 5. Potential Improvements
*   **Real Backend:** Move `portalService` logic to an actual Express.js server as originally requested, using `axios` to bridge the frontend and backend.
*   **Websockets:** Implement `socket.io` for real-time message updates instead of polling or manual refresh.
*   **File Upload:** Implement actual file upload to S3/ServiceM8 using Signed URLs.

## 6. Setup Instructions
1.  Run `npm install` (or equivalent in the sandbox).
2.  Ensure `process.env.API_KEY` is available for the Gemini features.
3.  The app auto-logs in with demo credentials for ease of testing.
